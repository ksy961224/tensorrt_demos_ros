...
class Camera():
    ...
    def _open(self):
        ...
        if a.image:
            ...
            self.cap = open_cam_onboard(1280,720)
            self._start()
        else:
    ...
    def read(self):
        ...
        if self.video_file:
            ...
            if self.copy_frame:
                ...
                mtx=np.array([772.9207765735987, 0, 621.139143406166, 0, 772.216150674872, 343.4904803667087, 0, 0, 1]).reshape([3,3])
                dist=np.array([-0.3094298605004002, 0.07978453774785584, 0.0002989143498620896, -0.0005130578027075569, 0]).reshape([1,5])
                newcameramtx=np.array([570.2941284179688, 0, 607.7548013146807, 0, 0, 715.08544921875, 340.856837970714, 0, 0, 0, 1, 0]).reshape([3,4])
                # self.img_handle=cv2.undistort(self.img_handle,mtx,dist,None,newcameramtx)
                self.img_handle=self.img_handle[:,159:159+960]
                self.img_handle = cv2.resize(self.img_handle, (640, 480))
                # print(self.img_handle.shape[:2])
